...
class Plankton {
  ...
  display() {
    ...
    image(planktonImg, (this.x + dx) * this.size, (this.y + dy) * this.size, this.size, this.size); // Draw plankton image
  }

  eat(chlorophylls) {
    for (let i = chlorophylls.length - 1; i >= 0; i--) {
      if (dist(this.targetX, this.targetY, chlorophylls[i].x, chlorophylls[i].y) < 1) {
...
class Chlorophyll {
  ...
  display() {
    ...
    ellipse(this.x * (width / maze[0].length) + (width / maze[0].length) / 2,
            this.y * (width / maze[0].length) + (width / maze[0].length) / 2,
            this.size); // Draw chlorophyll as a green circle
  }
}
...
class Cloud {
  ...
  display() {
    image(cloudImg, this.x * width / maze[0].length, this.y * width / maze[0].length, this.size, this.size); // Draw cloud image
  }
